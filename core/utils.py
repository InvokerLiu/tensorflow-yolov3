...
def draw_boxes(image, boxes, scores, labels, classes, detection_size,
    ...
    # draw settings
    font = ImageFont.truetype(font=font, size=np.floor(8e-2 * image.size[1]).astype('int32'))
    ...
    for i in range(len(labels)): # for each bounding box, do:
        ...
        draw.rectangle(bbox, outline=colors[labels[i]], width=1)
...
def compute_accuracy(y_pred, y_true, iou_thresh=0.5, score_thresh=0.3):
    num_images = y_true[0].shape[0]
    num_classes = y_true[0][0][..., 5:].shape[-1]
    acc = 0
    ...
    for i in range(num_images):
        ...
        if pred_boxes is not None and len(true_boxes_list) == len(pred_boxes):
            c_l = np.zeros((len(true_boxes_list), 2))
            for j in range(len(true_boxes_list)):
                c_l[j][0] = pred_boxes[j][0]
                c_l[j][1] = pred_labels[j]
            label_x_sort = np.argsort(c_l[:, 0])
            c_l = c_l[label_x_sort]
            temp_count = 0
            for j in range(len(true_boxes_list)):
                if true_labels_list[j] == c_l[j][1]:
                    temp_count += 1
            if temp_count == len(true_labels_list):
                acc += 1
    acc /= num_images
    return acc
